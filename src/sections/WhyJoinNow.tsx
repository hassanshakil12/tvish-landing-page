const WhyJoinNow = () => {
  const benefits = [
    ...
    },
    {
      ...
      description: "Keep 90% of your earnings as a Silver & Gold founder",
    },
    {
      icon: "⭐",
  ...
  return (
    <div className="w-full min-h-fit bg-linear-to-bl from-black via-black to-blue-950/50 px-4 sm:px-6 lg:px-8 py-12 lg:py-20">
      {/* Header Section */}
      ...
      <div className="w-full max-w-[80vw] mx-auto mb-16 lg:mb-20">
        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-5 gap-6 lg:gap-8">
          {benefits.map((benefit, index) => (
            <div
              key={index}
              className="group relative bg-linear-to-br from-black/80 to-blue-950/40 border-2 border-blue-900/40 rounded-3xl p-6 hover:border-blue-500/60 transition-all duration-700 hover:transform hover:scale-105 cursor-pointer overflow-hidden"
            >
              {/* Icon Container with Enhanced Effects */}
              <div className="relative mb-6">
                <div className="absolute inset-0 bg-blue-500/20 rounded-full blur-lg group-hover:blur-xl transition-all duration-500 scale-0 group-hover:scale-100"></div>
              ...
              <div className="relative z-10">
                <h3 className="text-xl lg:text-2xl font-bold text-white text-center mb-4 leading-tight bg-linear-to-r from-white to-gray-300 bg-clip-text group-hover:from-blue-400 group-hover:to-blue-200 transition-all duration-500">
                  {benefit.title}
                </h3>
              ...
              <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 w-2 h-2 bg-blue-500 rounded-full opacity-0 group-hover:opacity-100 group-hover:animate-ping transition-opacity duration-500"></div>
            </div>
          ))}
        </div>
      </div>
